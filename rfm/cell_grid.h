...
#ifndef RFM_CELL_GRID_H
#define RFM_CELL_GRID_H
...
public:
     ...
     */
    explicit CellGrid(double metersPerCell);
    ...
    CellGrid(CellGrid&& toMove) noexcept;
    CellGrid& operator=(const CellGrid& rhs);
    CellGrid& operator=(CellGrid&& rhs) noexcept;
    ~CellGrid();

    // Methods for accessing grid parameters
    std::size_t getWidthInCells() const { return width_; }
    float getWidthInMeters() const { return width_ * metersPerCell_; }

    std::size_t getHeightInCells() const { return height_; }
    float getHeightInMeters() const { return height_ * metersPerCell_; }

    Rectangle<float> getBoundary() const { return boundary_; }

    double metersPerCell() const { return metersPerCell_; }
    double cellsPerMeter() const { return cellsPerMeter_; }

    Point<float> getGlobalCenter() const;
    Point<float> getBottomLeft() const { return boundary_.bottomLeft; }
     ...
     */
    void changeBoundary(const Rectangle<float>& newBoundary, T initialValue = T());
...
private:
    ...
    Rectangle<float> boundary_;   ///< Global metric boundary of the area covered by the grid
...
template <typename T>
CellGrid<T>::CellGrid(CellGrid<T>&& toMove) noexcept
...
template <typename T>
CellGrid<T>& CellGrid<T>::operator=(CellGrid<T>&& rhs) noexcept
{
...
// Methods for mutating the grid
template <typename T>
Point<float> CellGrid<T>::getGlobalCenter() const
{
...
// Methods for modifying position of the grid
template <typename T>
void CellGrid<T>::changeBoundary(const Rectangle<float>& newBoundary, T initialValue)
{
