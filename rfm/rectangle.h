...
#ifndef RFM_RECTANGLE_H
#define RFM_RECTANGLE_H
...
public:
    ...
    Rectangle() : topLeft(0, 1), topRight(1, 1), bottomLeft(0, 0), bottomRight(1, 0) { }
    ...
    bool contains(double x, double y) const { return contains(Point<double>(x, y)); }
    ...
    bool contains(const Point<T>& point) const
    {
         ...
         * using the left_of_line function with the endpoints moving clockwise around the perimeter
         * of the Rectangle.
         */
     ...
     */
    bool intersections(const Line<T>& line, std::vector<Point<T>>& inters) const
    {
     ...
     */
    double distanceToPoint(const Point<T>& point) const
    {
        if (contains(point)) {
            return 0.0;
        }
        ...
        for (const auto& edge : edges) {
            auto closest = closest_point_on_line_segment(point, edge);
            double distance =
    ...
    void rotate(double radians)
    {
        ...
        bottomLeft = rotate(bottomLeft - c, radians) + c;
        bottomRight = rotate(bottomRight - c, radians) + c;
        topLeft = rotate(topLeft - c, radians) + c;
        topRight = rotate(topRight - c, radians) + c;
    }
     ...
     * right points.
     */
    float width() const { return distance_between_points(bottomLeft, bottomRight); }

    /**
     * height calculates the height of the rectangle, as determined by the distance between the bottom left and top left
     * points.
     */
    float height() const { return distance_between_points(bottomLeft, topLeft); }
