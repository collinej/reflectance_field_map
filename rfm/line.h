...
struct Line
{
    ...
    template <typename U>
    explicit Line(const Line<U>& copy) : a(copy.a)
                              , b(copy.b)
    {
    }
...
// Helper class that handles the common calculation between all the line/line segment intersection algorithms
template <typename T>
struct line_intersection_data_t
{
    ...
    bool linesIntersect() const { return den != 0; }

    bool segmentsIntersect() const
    {
        return linesIntersect() && ((rNum / den) >= -0.0000001) && ((rNum / den) <= 1.0000001)
          && ((sNum / den) >= -0.0000001) && ((sNum / den) <= 1.0000001);
    }

    Point<T> intersectionPoint() const
    {
...
bool line_segments_intersect(const Line<T>& lineA, const Line<U>& lineB)
{
    ...
    // If den == 0, then see if the x-projection of the lines overlap at all, i.e. collinear lines -- yes, very heinous
    // looking equation
