...
: DiscretizedAngleGrid(100, 100, metersPerCell, Point<float>(0, 0), activeRadius, numAngleBins, std::move(name))
{
}
...
void DiscretizedAngleGrid::changeBoundary(const Rectangle<double>& newBoundary)
{
...
bool DiscretizedAngleGrid::needToRecenterActiveRegion(Point<float> robotPosition)
{
...
{
    if (!needToRecenterActiveRegion(robotPosition)) {
        return false;
    }
...
{
    return cells_.isCellInGrid(x, y) && (cells_(x, y) != kUnassignedBin);
}


DiscretizedAngleGrid::TileIter DiscretizedAngleGrid::beginActiveTiles()
{
    return TileIter(activeRegion_.bottomLeft.x, activeRegion_.bottomLeft.y, activeWidth(), cells_);
}


DiscretizedAngleGrid::TileIter DiscretizedAngleGrid::endActiveTiles()
{
    return TileIter(activeRegion_.bottomLeft.x, activeRegion_.bottomLeft.y + activeHeight(), activeWidth(), cells_);
}


void DiscretizedAngleGrid::reset()
{
...
void DiscretizedAngleGrid::flush()
{
...
std::size_t DiscretizedAngleGrid::inMemoryFootprint() const
{
    return (angleBins_.size() * sizeof(Value)) + (availableBins_.size() * sizeof(BinIndex))
      + (cells_.getHeightInCells() * cells_.getWidthInCells() * sizeof(BinIndex));
}


std::size_t DiscretizedAngleGrid::onDiskFootprint() const
{
    return cachedTilePositions_.size() * kAngleBinsPerTile_ * sizeof(Value);
}
...
DiscretizedAngleGrid::TileRegion DiscretizedAngleGrid::calculateActiveRegion(Point<float> center)
{
    ...
    return TileRegion(
      Tile(std::clamp(centerTile.x - activeRegionRadius_, 0, static_cast<int>(cells_.getWidthInTiles() - 1)),
           std::clamp(centerTile.y - activeRegionRadius_, 0, static_cast<int>(cells_.getHeightInTiles() - 1))),
      Tile(std::clamp(centerTile.x + activeRegionRadius_, 0, static_cast<int>(cells_.getWidthInTiles() - 1)),
           std::clamp(centerTile.y + activeRegionRadius_, 0, static_cast<int>(cells_.getHeightInTiles() - 1))));
}


void DiscretizedAngleGrid::setActiveBins()
{
    for (auto activeIt = beginActiveTiles(), activeEnd = endActiveTiles(); activeIt != activeEnd; ++activeIt) {
        assignTileBins(Tile(activeIt.x(), activeIt.y()));
    }
...
void DiscretizedAngleGrid::eraseOffMapTiles()
{
...
void DiscretizedAngleGrid::resetCachedTiles()
{
...
void DiscretizedAngleGrid::tileWrite(Tile tile)
{
    ...
    if (fp) {
        ...
        if (compression) {
            std::cout << "STUB! " << "DiscretizedAngleGrid::tileWrite compressed data" << ": " << __FILE__ << ':' << __LINE__ << std::endl;
...
void DiscretizedAngleGrid::tileRead(Tile tile)
{
    ...
    if (fp) {
        ...
        if (!compression) {
            ...
            std::cout << "STUB! " << "DiscretizedAngleGrid::tileRead compressed data" << ": " << __FILE__ << ':' << __LINE__ << std::endl;
...
void DiscretizedAngleGrid::verifyActiveRegion()
{
    for (auto activeIt = beginActiveTiles(), activeEnd = endActiveTiles(); activeIt != activeEnd; ++activeIt) {
        if (*activeIt == kUnassignedBin) {
...
std::string tile_name(const std::string& gridName, Point<double> tilePosition)
{
    ...
    filename << kTileBaseDirectory << gridName << "/Tile_X-" << xName << "_Y-" << yName << "_Sz-" << DiscretizedAngleGrid::kTileSize
             << ".tile";
    return filename.str();
}
