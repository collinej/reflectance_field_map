...
#ifndef RFM_DISCRETIZED_ANGLE_GRID_H
#define RFM_DISCRETIZED_ANGLE_GRID_H

#include "point.h"
#include "grid_iterators.h"
#include "tiled_cell_grid.h"
...
public:
    ...
    using TileIter = TiledGridIterator<int64_t, TiledCellGrid<int64_t, kTileSize>>;
    ...
    std::size_t getWidthInCells() const { return cells_.getWidthInCells(); }
    double getWidthInMeters() const { return cells_.getWidthInMeters(); }

    std::size_t getHeightInCells() const { return cells_.getHeightInCells(); }
    double getHeightInMeters() const { return cells_.getHeightInMeters(); }

    std::size_t numAngleBins() const { return kAngleBinsPerCell_; }

    float metersPerCell() const { return cells_.metersPerCell(); }
    float cellsPerMeter() const { return cells_.cellsPerMeter(); }

    Rectangle<double> getBoundary() const { return cells_.getBoundary(); }
    Point<double> getGlobalCenter() const { return cells_.getGlobalCenter(); }
    Point<double> getBottomLeft() const { return cells_.getBottomLeft(); }
     ...
     */
    void changeBoundary(const Rectangle<double>& newBoundary);
     ...
     */
    bool needToRecenterActiveRegion(Point<float> robotPosition);
     ...
     */
    Rectangle<float> getActiveRegion() const;
    Rectangle<int> getActiveRegionInCells() const;
    double activeRegionRadius() const { return activeRegionRadius_ * cells_.metersPerTile(); }
    Point<float> activeRegionCenter() { return activeCenter_; }
     ...
     * This iterator is invalidated if a call to recenterActiveRegion returns true or by a call to changeBoundary.
     */
    TileIter endActiveTiles();

    /**
     * numActiveTiles retrieves the number of active tiles.
     */
    std::size_t numActiveTiles() const { return activeRegion_.width() * activeRegion_.height(); }
    ...
    /**
     * Compute the current in-memory size of the grid.
     */
    std::size_t inMemoryFootprint() const;

    /**
     * Compute the overall on-disk size of the grid.
     */
    std::size_t onDiskFootprint() const;

private:
    using Tile = Point<int>;
    using TileRegion = Rectangle<int>;
...
#endif   // RFM_DISCRETIZED_ANGLE_GRID_H
